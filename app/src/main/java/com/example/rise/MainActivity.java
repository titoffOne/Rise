...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    // Метод, передающий данные будильника в класс MyAlarm
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public void setMyAlarm(){
        int hour = dataBaseManager.getSettingsAlarm().get(0).getHour();
        int minute = dataBaseManager.getSettingsAlarm().get(0).getMinute();
        String name = dataBaseManager.getSettingsAlarm().get(0).getName();
        int sound = dataBaseManager.getSettingsAlarm().get(0).getSound();
        int vibration = dataBaseManager.getSettingsAlarm().get(0).getVibration();
        myAlarm = new MyAlarm(this, hour, minute, name, sound, vibration);
        myAlarm.createAlarm();
